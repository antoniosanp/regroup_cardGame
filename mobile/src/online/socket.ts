...
// fake socket in tests (no live backend needed to verify the contract). Same
// interface as the web client's socket; the only RN-specific parts are the
// binary-frame options (React Native's WebSocket mangles the NULL byte STOMP
// uses as a frame terminator when frames are sent as strings).
