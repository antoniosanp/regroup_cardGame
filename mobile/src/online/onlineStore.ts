...
// stats are mirrored in full for every player. Direct port of the web client's
// store; the only difference is that identity persistence is async here.
  ...
  const store = createStore<OnlineState>((set, get) => {
    ...
    function connect(identity: Identity): void {
      ...
      socket.activate(identity.token, {
        onConnect: () => {
          const s = get();
          set({ conn: 'connected', stage: s.stage === 'name' ? 'lobby' : s.stage });
          if (s.matchId) {
        ...
        onDisconnect: () => {
          if (get().conn === 'connected') set({ conn: 'reconnecting' });
        },
